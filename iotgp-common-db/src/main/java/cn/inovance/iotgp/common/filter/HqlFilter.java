 ...
 * //NUL is null // NULL is not null // LLK 左模糊
 * 
 */
public class HqlFilter {
	...
	private String getSqlOperator(String operator) {
		...
		if (StringUtils.equalsIgnoreCase(operator, "NUL")) {
			return " is null";
		}
		if (StringUtils.equalsIgnoreCase(operator, "NNUL")) {
			return " is not null";
		}
	 ...
	 * 获取查询条件的or 或者 and
	 */
	public String getAndOr(String or) {
		if (StringUtils.equalsIgnoreCase(or, "OR")) {
			return " or ";
		} else {
			return " and ";
		}
		...
		if (name != null && value != null) {
			if (name.startsWith("QUERY_")) {// 如果有需要过滤的字段
				...
				if (filterParams.length == 4) {
					...
					String columnType = filterParams[2];// 字段类型
					String operator = filterParams[3];// SQL操作符
					String or = filterParams[4]; // or is and
					String placeholder = UUID.randomUUID().toString()
							.replace("-", "");// 生成一个随机的参数名称
					...
					hql.append(getAndOr(or) + columnName + " "
							+ getSqlOperator(operator) + " :param"
							+ placeholder + " ");// 拼HQL
					params.put("param" + placeholder,
							getObjValue(columnType, operator, value));// 添加参数
